...
EmulatorWebServer::EmulatorWebServer(uint16_t port) : _server(port) {}
...
void EmulatorWebServer::loop() { ElegantOTA.loop(); }
...
void EmulatorWebServer::setupRoutes() {
  ...
  _server.on(
      "/emulator/status", HTTP_GET,
      [this](AsyncWebServerRequest *request) { handleGetStatus(request); });

  _server.on(
      "/emulator/monitored", HTTP_GET,
      [this](AsyncWebServerRequest *request) { handleGetMonitored(request); });

  _server.on(
      "/emulator/emulated", HTTP_GET,
      [this](AsyncWebServerRequest *request) { handleGetEmulated(request); });

  _server.on("/system_status", HTTP_GET,
  ...
  _server.on("/emulator/door/position", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleDoorSetPosition(request);
             });

  _server.on(
      "/emulator/door/open", HTTP_POST,
      [this](AsyncWebServerRequest *request) { handleDoorSetOpen(request); });

  _server.on(
      "/emulator/door/close", HTTP_POST,
      [this](AsyncWebServerRequest *request) { handleDoorSetClosed(request); });

  _server.on("/emulator/door/fault", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleDoorInjectFault(request);
             });

  _server.on("/emulator/door/clear_fault", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleDoorClearFault(request);
             });

  _server.on(
      "/emulator/door/config", HTTP_POST,
      [this](AsyncWebServerRequest *request) { handleDoorConfig(request); });
  ...
  _server.on(
      "/emulator/water/config", HTTP_POST,
      [this](AsyncWebServerRequest *request) { handleWaterConfig(request); });

  _server.on(
      "/emulator/water/pulse", HTTP_POST,
      [this](AsyncWebServerRequest *request) { handleWaterPulse(request); });

  _server.on("/emulator/water/reset", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleWaterResetCounters(request);
             });

  _server.on("/emulator/water/frozen", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
  ...
  _server.on("/emulator/manual_switch/press", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleManualSwitchPress(request);
             });

  _server.on("/emulator/manual_switch/release", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleManualSwitchRelease(request);
             });

  _server.on("/emulator/manual_switch/pulse", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleManualSwitchPulse(request);
             });

  _server.on("/emulator/manual_switch/long_press", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleManualSwitchLongPress(request);
             });

  _server.on("/emulator/manual_switch/config", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
  ...
  _server.on("/emulator/override/enable", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleOverrideEnable(request);
             });

  _server.on("/emulator/override/disable", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleOverrideDisable(request);
             });

  _server.on("/emulator/override/set", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleOverrideSetState(request);
             });

  _server.on("/emulator/override/clear_all", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
  ...
  _server.on(
      "/emulator/scenarios", HTTP_GET,
      [this](AsyncWebServerRequest *request) { handleGetScenarios(request); });

  _server.on("/emulator/scenario/active", HTTP_GET,
             [this](AsyncWebServerRequest *request) {
               handleGetActiveScenario(request);
             });

  _server.on("/emulator/scenario/apply", HTTP_POST,
             [this](AsyncWebServerRequest *request) {
               handleApplyScenarioById(request);
             });

  _server.on(
      "/emulator/scenario/custom", HTTP_POST,
  ...
  _server.on(
      "/emulator/fault/door_stuck", HTTP_POST,
      [this](AsyncWebServerRequest *request) { handleSetDoorStuck(request); });
  ...
  _server.on(
      "/update_settings", HTTP_POST, [](AsyncWebServerRequest *request) {},
      nullptr,
      [this](AsyncWebServerRequest *request, uint8_t *data, size_t len,
             size_t index,
             size_t total) { handleUpdateSettings(request, data, len); });

  _server.on("/emulator/settings/export", HTTP_GET,
             [this](AsyncWebServerRequest *request) {
               handleExportSettings(request);
             });

  _server.on(
      "/emulator/settings/import", HTTP_POST,
      [](AsyncWebServerRequest *request) {}, nullptr,
      [this](AsyncWebServerRequest *request, uint8_t *data, size_t len,
             size_t index,
             size_t total) { handleImportSettings(request, data, len); });

  // ========================================================================
  // SYSTEM
  // ========================================================================

  _server.on("/reboot", HTTP_POST,
             [this](AsyncWebServerRequest *request) { handleReboot(request); });

  _server.on(
      "/factory_reset", HTTP_POST,
  ...
  _server.onNotFound([](AsyncWebServerRequest *request) {
    if (request->method() == HTTP_GET) {
      request->send(LittleFS, "/www/index.htm", "text/html");
    } else {
...
void EmulatorWebServer::handleDoorConfig(AsyncWebServerRequest *request) {
  ...
  if (request->hasParam("travel_time_ms", true)) {
    config.doorTravelTimeMs =
        request->getParam("travel_time_ms", true)->value().toInt();
  }
  if (request->hasParam("auto_simulate", true)) {
    config.autoSimulateDoor =
        request->getParam("auto_simulate", true)->value() == "true";
  }
...
void EmulatorWebServer::handleWaterConfig(AsyncWebServerRequest *request) {
  ...
  if (request->hasParam("pulses_per_gallon", true)) {
    config.pulsesPerGallon =
        request->getParam("pulses_per_gallon", true)->value().toFloat();
  }
  if (request->hasParam("flow_rate_gpm", true)) {
    config.flowRateGPM =
        request->getParam("flow_rate_gpm", true)->value().toFloat();
  }
  if (request->hasParam("auto_generate", true)) {
    config.autoGeneratePulses =
        request->getParam("auto_generate", true)->value() == "true";
  }
...
void EmulatorWebServer::handleWaterResetCounters(
    AsyncWebServerRequest *request) {
  _stateManager->resetPulseCounters();
  sendSuccessResponse(request, "Pulse counters reset");
}

void EmulatorWebServer::handleWaterSimulateFrozen(
    AsyncWebServerRequest *request) {
  bool frozen = true;
  ...
  _stateManager->setConfig(config);
  sendSuccessResponse(request, frozen ? "Frozen line simulation ON"
                                      : "Frozen line simulation OFF");
}
...
void EmulatorWebServer::handleManualSwitchLongPress(
    AsyncWebServerRequest *request) {
...
void EmulatorWebServer::handleManualSwitchConfig(
    AsyncWebServerRequest *request) {
...
void EmulatorWebServer::handleOverrideSetState(AsyncWebServerRequest *request) {
  ...
  if (request->hasParam("hall_open", true)) {
    _stateManager->setOverrideHallOpen(
        request->getParam("hall_open", true)->value() == "true");
  }
  if (request->hasParam("hall_close", true)) {
    _stateManager->setOverrideHallClose(
        request->getParam("hall_close", true)->value() == "true");
  }
  if (request->hasParam("door_fault", true)) {
    _stateManager->setOverrideDoorFault(
        request->getParam("door_fault", true)->value() == "true");
  }
  if (request->hasParam("manual_switch", true)) {
    _stateManager->setOverrideManualSwitch(
        request->getParam("manual_switch", true)->value() == "true");
  }
  if (request->hasParam("water_pulse_1", true)) {
    _stateManager->setOverrideWaterPulse(
        1, request->getParam("water_pulse_1", true)->value() == "true");
  }
  if (request->hasParam("water_pulse_2", true)) {
    _stateManager->setOverrideWaterPulse(
        2, request->getParam("water_pulse_2", true)->value() == "true");
  }
...
void EmulatorWebServer::handleGetActiveScenario(
    AsyncWebServerRequest *request) {
  JsonDocument doc;
  JsonObject obj = doc.to<JsonObject>();
...
void EmulatorWebServer::handleApplyScenarioById(
    AsyncWebServerRequest *request) {
...
void EmulatorWebServer::handleApplyCustomScenario(
...
void EmulatorWebServer::handleSetDoorStuck(AsyncWebServerRequest *request) {
  ...
  _stateManager->setConfig(config);
  sendSuccessResponse(request, stuck ? "Door stuck simulation ON"
                                     : "Door stuck simulation OFF");
}
...
void EmulatorWebServer::handleUpdateSettings(AsyncWebServerRequest *request,
...
void EmulatorWebServer::handleExportSettings(AsyncWebServerRequest *request) {
  JsonDocument doc;
  JsonObject obj = doc.to<JsonObject>();

  // Include all settings (including password) for backup
  emulatorSettings.toJson(obj, true);

  // Add metadata for the export
  obj["_export_version"] = "1.0";
  obj["_export_timestamp"] = millis();
  obj["_device_hostname"] = hostName;
  ...
  response->addHeader("Content-Disposition",
                      "attachment; filename=\"emulator_settings_backup.json\"");
  response->addHeader("Cache-Control", "no-cache");
  request->send(response);
}

void EmulatorWebServer::handleImportSettings(AsyncWebServerRequest *request,
                                             uint8_t *data, size_t len) {
  JsonDocument doc;
  DeserializationError error = deserializeJson(doc, data, len);
  ...
  // Validate that this is a settings export (check for expected fields)
  if (!obj["ssid"].is<const char *>() &&
      !obj["door_travel_time_ms"].is<int>() &&
      !obj["_export_version"].is<const char *>()) {
    sendErrorResponse(request, "Invalid settings file format");
    return;
  }

  // Apply the imported settings
  if (!emulatorSettings.fromJson(obj)) {
    sendErrorResponse(request, "Failed to parse settings");
    return;
  }

  // Save to filesystem
  if (!emulatorSettings.save()) {
    sendErrorResponse(request, "Failed to save settings to filesystem");
    return;
  }
  ...
  response["requires_reboot"] = true; // WiFi settings may have changed
  sendJsonResponse(request, response);
}
...
void EmulatorWebServer::sendJsonResponse(AsyncWebServerRequest *request,
...
void EmulatorWebServer::sendErrorResponse(AsyncWebServerRequest *request,
...
void EmulatorWebServer::sendSuccessResponse(AsyncWebServerRequest *request,
                                            const char *message) {
  JsonDocument doc;
  doc["message"] = message;
  doc["success"] = true;
